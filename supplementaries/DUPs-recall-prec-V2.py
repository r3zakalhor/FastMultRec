...
def process_subdir(subdir, t1, t2):
    ...
    for algorithm in ['lca'] + [f'{alg}{i}' for alg in ['greedy', 'greedydown'] for i in i_values]:
        variable_file = os.path.join(subdir, f'out_{algorithm}_NNI_K15.txt.csv')
...
def generate_csv(directory, t1, t2):
    ...
    algorithms = ['lca'] + [f'{alg}{i}' for alg in ['greedy', 'greedydown'] for i in [5, 100]]
    
    for metric in metrics:
        metric_df = df[['sim_number'] + [f'{algorithm}_{metric}' for algorithm in algorithms]]
        output_file = f"{os.path.basename(directory)}-{metric}-3W-NNI-K15-WGD-t20-t80.csv"
