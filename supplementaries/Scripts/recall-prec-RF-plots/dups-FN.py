...
def calculate_accuracy(reference, target, threshold=0.8):
    ...
    # Filter species with Gis > 10 in reference
    ...
    accuracy = sum((filtered_reference['Num Gis'] > filtered_target['Num Gis']) & 
    ...
    if total_count > 0:
        accuracy_percentage = (1 - (accuracy / total_count)) * 100
    else:
...
def generate_csv(directory):
    ...
    output_file = f"{os.path.basename(directory)}-FN-WGD.csv"
