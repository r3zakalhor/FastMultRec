...
def process_subdir(subdir, t1, t2):
    ...
    for algorithm in ['lca'] + [f'{alg}{i}' for alg in ['greedydown'] for i in i_values]:
        variable_file = os.path.join(subdir, f'out_true_{algorithm}.txt.csv')
...
def generate_csv(directory, t1, t2):
    ...
    algorithms = ['lca'] + [f'{alg}{i}' for alg in ['greedydown'] for i in [3, 10, 50, 60, 70, 80, 90, 100]]
    
    for metric in metrics:
        metric_df = df[['sim_number'] + [f'{algorithm}_{metric}' for algorithm in algorithms]]
        output_file = f"{os.path.basename(directory)}-{metric}-0W-WGD-t20-t80.csv"
