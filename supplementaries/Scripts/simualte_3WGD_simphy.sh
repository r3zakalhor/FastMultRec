...
dir_name="WGD3Simphy_S1_G100_Duprate_u-gb_1"
...
do
	...
	python simphy_wgd.py -spfile=s_tree_modded2.trees -auxfile=myaux3.txt -sout=s_tree_modded3.trees
	s_tree_modded=$(cat s_tree_modded3.trees)
	./simphy -sb f:0.000001 -gb u:-25,-15 -gt f:0.0 -gg f:0.0 -gp f:0.0 -ld ln:gb,1 -lb f:ld -lt f:0.0 -lg f:0.0 -rs 1 -rl f:99 -rg 1 -o $dir_name -sp f:2 -su f:0.00001 -sg f:1 -sl U:50,110 -st f:1000000 -om 1 -v 3 -od 1 -op 1 -oc 1 -on 1 -ol 1 -cs $RANDOM -S $s_tree_modded
	...
	do
		...
		echo $filename
		python simphy_wgd.py -mode=remap -auxfile=myaux3.txt -slfile=$dir_name/$newname/$filename -lgfile=$dir_name/$newname/$filename2 -genetreefile=$dir_name/$newname/$filename3 > out.txt
	done
	...
	do
    		ii=$(printf "%02d" $j)
		filename="${ii}.mapsl.modded.modded"
		filename2="${ii}l1g.maplg.modded.modded"
		filename3="g_trees${ii}.trees.modded.modded"
		echo $filename
	...
	cd $newname 
	cat g_trees*.modded.modded.modded >> all_genetrees.txt
	...
	cp myaux3.txt $dir_name/$newname/myaux3.txt
	...
	cp s_tree_modded3.trees $dir_name/$newname/s_tree_modded3.trees
	...
	if [ -s $dir_name/$newname/s_tree.trees ]; then
		...
		python map_gene_trees_threeWGD.py $dir_name/$newname/all_genetrees.txt $dir_name/$newname/all_genetrees_edited.txt $dir_name/$newname
	fi
